...
- 30분마다 원문을 다시 읽되, 본문을 사실상 다시 쓴 대규모 개편만 변경 알림
- 기사 링크보다 공식 신청·접수 링크를 우선 탐색
- 원문에 명시된 신청서, 동의서, 발표자료, 소스 코드 등을 준비 체크리스트로 생성
- 서울·경기 이외 지역 거주자·재학생·소재 기업만 참가할 수 있는 지역 한정 행사는 제외
...
- 국내 기회정보는 한국어 검색 RSS에서 후보를 찾고 원문과 신청 링크를 읽은 뒤, 마감된 행사·내부 행사·단순
  홍보 기사와 서울·경기 외 지역 한정 행사를 제외합니다. 지방 개최 행사도 전국 누구나 참가 가능하면 유지합니다.
  원문에 없는 접수일/상금/참가 조건은 추측하지 않고 `확인 필요`로 표시합니다.
